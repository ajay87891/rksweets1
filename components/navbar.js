  ...
  return (
    ...
    <nav className="w-full flex justify-between items-center  bg-white/20 fixed top-0 backdrop-blur-sm px-10 z-40 py-4">
          ...
          (item) => {
            return (
              <li className="cursor-pointer flex flex-col group items-center" key={`link-${item}`}>
                ...
                <a className="text-violet-500 font-medium 2xl:text-2xl font-Quicksand group-hover:text-violet-700 ease-in-out duration-500 transition-all uppercase" href={`#${item}`}>{item}</a>
      ...
      <div className="md:hidden ">
        ...
        {
          toggel ? (<motion.div className=" opacity-0 fixed z-50 top-0 bottom-0 right-0 p-4 w-[80%] h-[100vh] flex  flex-col  bg-violet-100 shadow-lg bg-cover bg-repeat "
          style={{
          ...
          (item) => {
            return (
              <li key={item}>
                <a className="text-violet-500 text-2xl cursor-pointer font-medium font-Quicksand hover:text-violet-700 ease-in-out duration-500 transition-all uppercase" href={`#${item}`} onClick={()=> setToggel(false)}>{item}</a>
              </li>
            );
          }
        )}

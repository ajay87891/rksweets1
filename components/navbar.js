    ...
    <nav className="w-full flex justify-between items-center  bg-primary/80 fixed top-0 backdrop-blur-sm px-2 z-40 py-3 lg:py-4">
      ...
      <div className="hidden md:block ">
        <ul className="flex space-x-8 font-Coiny">
          {menuItems.map((item) => {
            return (
              <li
                className="cursor-pointer flex flex-col group items-center"
                key={`link-${item}`}
              >
      ...
      <div className="md:hidden ">
        <button
          ...
          className="inline-flex items-center p-2 ml-3 text-sm text-violet-500 rounded-lg md:hidden hover:bg-violet-100 focus:outline-none focus:ring-2 focus:ring-violet-200     ease-linear duration-500"
        >
          <svg
            className="w-6 h-6"
            aria-hidden="true"
